...
router.get("/destinations/countries", async (req, res) => {
  try {
    ...
    // Use your existing REST Countries v5 API if key exists.
    // Otherwise use public REST Countries v3 fallback.
    ...
    const eventResults = process.env.TICKETMASTER_API_KEY
      ? await Promise.allSettled(
          eventCountrySeeds.map((countryCode) =>
            fetchTicketmasterEventsByCountry(countryCode, 4, startDate, endDate)
          )
        )
      : [];
